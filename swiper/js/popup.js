$(document).ready(function(){
    const swiper = new Swiper('.visual1', {

        loop: true, //계속 돌아가게함 - 마지막 팝업에서 1번 팝업으로 순환 (지우면 마지막 팝업에서 슬라이드가 안됨)
        effect: "fade", //fade 효과로 팝업 넘김(지우면 좌우 슬라이드)


        autoplay: {/* 자동실행 */
            delay: 3000, //하나의 팝업이 보여지는 시간
            disableOnInteraction: false,
        },
      
        // 팝업의 숫자대로 동그라미를 그려주거나 2/5 이런 식의 팝업 수를 표시해주는 요소
        pagination: {
          el: '.ctrl_paging',
          //clickable: true, 동그라미일때 동그라미를 클릭하게 해주는 것
          type: "fraction",//숫자로 팝업의 수 표현
        },
      
        // Navigation arrows
        navigation: {
          nextEl: 'ctrl_next', //다음버튼
          prevEl: 'ctrl_prev',
        },
        
      });

      $('.visual1 .ctrl_stop').on('click', function(){
        swiper.autoplay.stop();  /* 일시정지 기능 */
      });
      $('.visual1 .ctrl_play').on('click', function(){
        swiper.autoplay.start();  /* 재생 기능 */
      });

});