$(document).ready(function(){
   ...
   $('.visual .popup .btn_stop').on('click', function(){
        ...
        if(popStatus == true){//버튼의 상태가 play모양 -현재정지상태-재생기능을 실행
            swiper.autoplay.start();  /* 재생 기능 */
            $(this).removeClass('play');
            $(this).text('일시정지');
        }else{//버튼의 상태가 일시정지 모양 - 현재재생상태 -일시정지기능을 실행
   ...
   /* 이미지가 스크롤될때 오브젝트 움직이는 효과
   움직이는 시작을 오브젝트가 화면에 나타나기 시작했을때부터 스크롤된 값을 계산해서 움직일 값으로 변환해줘야함.
   1. 브라우저가 스크롤되는 값 - $(window).scrollTop();
   2. 오브젝트가 화면 하단에서 나타나기 시작하는 값
   --offset().top - 상단 맨 위에서부터 오브젝트까지의 거리값
   offset().top과 $(window).scrollTop값이 같아지는 시기는 오브젝트가 화면 상단에 딱 붙었을때
   ...
   */
   ...
   //objParallax($('.sns p'), $('.sns p'), 'left', 0.2);움직이고 싶은 요소가 있으면 이 값만 고치면됨!!!!!!
    ...
    product 콘텐츠가 화면에 보이는 구간 2158 ~ 4767 
    .product .list가 페이지 상단에 도달했을때 : 콘텐츠 보일 시작점
    offset().top == 해당 콘텐츠가 브라우저 상단 위쪽에 닿을 정도의 스크롤값


    --처음에 tit이 나타나기전 (다른 콘텐츠와 같이 스크롤되어 따라올라옴)
    tit이 고정되는 영역(고정되어 옆에 콘텐츠만 스크롤됨)-fixed 클래스 추가
    tit이 고전된 이후 영역(다른 콘텐츠를 따라서 사라짐)-end 클래스 추가
    */
    ...
    function objFixed(){
        ...
        }else if((scrolling >= fixStart)&&(scrolling < fixEnd)){//tit 고정
